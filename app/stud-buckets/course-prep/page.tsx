...
export const metadata: Metadata = {
  ...
  description: "Private Brewster Boys course intel, official tournament rules, player jobs and format-aware decisions.",
...
export default async function CoursePrepPage() {
  ...
  const authorized = verifyStudBucketsSession(captainSession) || verifyCourseIntelSession(courseSession);

  if (!authorized) return <AccessGate configured={courseIntelAccessConfigured()} scope="course" />;
    ...
    <div id="top" className="min-h-screen bg-[#f4f0e7] text-[#10201b]">
      <header className="sticky top-0 z-50 border-b border-white/8 bg-[#071b18]/95 text-white backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-3 px-4 py-3 sm:px-6">
          <Link href="#top" className="flex items-center gap-3">
            <Image src="/brand/brewster-boys-cutout.png" alt="" width={48} height={48} className="h-12 w-12 object-contain drop-shadow-[0_7px_10px_rgba(0,0,0,.35)]" />
          ...
          <div className="flex items-center gap-2">
            <nav className="hidden items-center gap-1 md:flex" aria-label="Field manual sections">
              <Link href="#official-rules" className="rounded-full bg-white/8 px-4 py-2 text-[9px] font-bold uppercase tracking-[0.14em] text-white/70 hover:bg-white/12">Official</Link>
      ...
      <footer className="bg-[#071b18] px-5 py-8 text-white">
        <div className="mx-auto max-w-6xl">
          ...
          <p className="mt-3 max-w-xl text-sm font-semibold text-white/70">Ten boys. One brew. Bring the damn thing home.</p>
          ...
          <p className="mt-2 max-w-3xl text-xs leading-5 text-white/32">
            ...
            {HOLE_PHOTO_SOURCES.map((source, index) => (
              <span key={source.courses}>{index > 0 ? " · " : ""}<a href={source.href} target="_blank" rel="noreferrer" className="underline decoration-white/25 underline-offset-2 hover:text-white/60">{source.courses}: {source.label}</a></span>
            ))}.
          </p>
        </div>
      </footer>
